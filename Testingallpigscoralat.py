...
output_folder = os.path.join(folder_path,'pngCOR','ECG')
os.makedirs(output_folder, exist_ok=True)  # creates folder if it doesn't exist
output_folder = os.path.join(folder_path,'pngCOR')
...
for filepath in PigCSVDataPathList:
    ...
    # Path to filtered respiration data files and list of bad channels
        ...
        plt.savefig(os.path.join( folder_path,'pngCOR','ECG', os.path.basename(filepath).split('.')[0]+'.png'), dpi=300, bbox_inches='tight')
        plt.clf()
        ...
        logging.info("good leads :" + str(good_leads))
        ...
        def select_peaks_near_extreme_amplitude(part, time_high_peaks, amplitude_percent=0.2):
            ...
            plt.savefig(os.path.join( folder_path,'pngCOR', os.path.basename(filepath).split('.')[0]+'.png'), dpi=300, bbox_inches='tight')
            plt.clf()
