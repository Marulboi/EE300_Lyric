...
output_folder = os.path.join(folder_path,outputName,'ECG')
os.makedirs(output_folder, exist_ok=True)  # creates folder if it doesn't exist
output_folder = os.path.join(folder_path,outputName)
...
prominenceForPeaks = 0.1 #0.0001
highttreshold = 0.4
msTime= 20 # 20
amplitudedetecet = True
respProminence = 0.2 #0.15
...
for filepath in PigCSVDataPathList:
    ...
    # Path to filtered respiration data files and list of bad channels
        ...
        time = np.linspace(0, df[0].size / sampling_rate, df[0].size)
        ...
        time_low_peaks, _ = find_peaks(
            ...
            prominence=prominenceForPeaks, 
            distance=min_distance_qrs_ms / 1000 * sampling_rate,
            height=highttreshold * np.max(-x)  # seuil minimum en valeur absolue
        )
        ...
        if len(time_low_peaks) > 0 and len(time_high_peaks) > 0:
            ...
            if len(time_low_peaks) > 0:
                plt.plot(time[time_low_peaks], x.iloc[time_low_peaks], "o")  # Troughs

            if len(time_high_peaks) > 0:
                plt.plot(time[time_high_peaks], x.iloc[time_high_peaks], "x")  # Peaks


        plt.plot(time, x)  # Raw signal
        plt.plot(time[time_low_peaks], x.iloc[time_low_peaks], "o")  # Troughs
        plt.plot(time[time_high_peaks], x.iloc[time_high_peaks], "x")  # Peaks
        plt.savefig(os.path.join( folder_path,outputName,'ECG', os.path.basename(filepath).split('.')[0]+'.png'), dpi=300, bbox_inches='tight')
        plt.clf()
        # Find and select leads for ECGI with the desired variances
        if len(time_low_peaks) == len(time_high_peaks) :
            if abs(max(x)) < abs(min(x)):
                df = -df
                time_high_peaks=time_low_peaks
        elif len(time_low_peaks) > len(time_high_peaks) :
            df = -df
            time_high_peaks=time_low_peaks
        ...
        nPoint50ms = int(msTime * sampling_rate / 1000)
        ...
        reference_track = savgol_filter(pca_RRamp[:,n_component], window_length=6, polyorder=2)# 6 , 2window_length=13, polyorder=9
        ...
        def select_peaks_near_extreme_amplitude(part, time_high_peaks, amplitude_percent=0.2):
            ...
            peaks, _ = find_peaks(resp_norm, prominence=respProminence, distance=500)
            troughs, _ = find_peaks(-resp_norm, prominence=respProminence, distance=500)
