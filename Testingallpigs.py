...
folder_path = r"C:\Users\emir.ege-nemutlu\Desktop\resp\Clinical\interpcsv"
output_folder = os.path.join(folder_path,'png','ECG')
os.makedirs(output_folder, exist_ok=True)  # creates folder if it doesn't exist
output_folder = os.path.join(folder_path,'png')
...
for filepath in PigCSVDataPathList:
    ...
    # Path to filtered respiration data files and list of bad channels
        ...
        df = df.T[start:]
        # Rows become channels, columns become time samples 
        ...
        min_distance_qrs_ms = 600  # Minimum time between two QRS complexes (in ms)
        ...
        median_index = np.argsort(peak_counts)[(len(peak_counts)*5// 6)]
